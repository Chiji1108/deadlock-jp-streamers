...
// Adapted from https://heatmap-shadcn.vercel.app/r/heatmap-calendar.json.
// Calendar dates are date-only values represented at UTC midnight, never instants.
...
export type AxisLabelsConfig = {
   ...
   * Default: [0,2,4] => Mon/Wed/Fri when weekStartsOn=1 (nice uncluttered)
   */
...
export type HeatmapCalendarProps = {
  ...
  getCellLabel?: (cell: HeatmapCell) => string;
  data: HeatmapDatum[];
  /** Number of days ending at endDate (default 365). Ignored when year mode is active. */
  rangeDays?: number;
  endDate?: Date; // UTC midnight representing a JST calendar date
  weekStartsOn?: 0 | 1;
...
function toKey(d: Date) {
  // Preserve the date-only keys supplied by Convex, independent of browser timezone.
...
function formatMonth(d: Date, fmt: "short" | "long" | "numeric") {
  if (fmt === "numeric") {
    ...
    return `${d.getUTCMonth() + 1}/${yy}`;
  }
  return d.toLocaleDateString("ja-JP", { timeZone: "UTC", month: fmt });
}
...
function weekdayLabelForIndex(index: number, weekStartsOn: 0 | 1) {
  ...
  return base
    .toLocaleDateString("ja-JP", { timeZone: "UTC", weekday: "short" })
    .toUpperCase();
}
...
export function HeatmapCalendar({
  ...
  const levels = levelClassNames ?? heatmapLevels;
  ...
  const weekdayIndices = axisCfg.weekdayIndices ?? [0, 2, 4];
  ...
  const [uncontrolledYear, setUncontrolledYear] = React.useState(
    () => defaultYear ?? year ?? today.getUTCFullYear(),
  );
  ...
  const end = yearModeActive
    ? startOfDay(new Date(Date.UTC(currentYear, 11, 31)))
    : startOfDay(endDate);
  const start = yearModeActive
    ? startOfDay(new Date(Date.UTC(currentYear, 0, 1)))
  ...
  const weekdayLabelWidth = showAxis && showWeekdays ? 32 : 0;
  const selected = cells.find(
    (cell) => cell.key === selectedCell && !cell.disabled,
  );
      ...
      <CardContent className="pt-1">
        <TooltipProvider delayDuration={80}>
          ...
          >
            ...
            <div className={cn("mx-auto w-max shrink-0", axisCfg.className)}>
              {/* Month labels row */}
              {showAxis && showMonths ? (
                <div
